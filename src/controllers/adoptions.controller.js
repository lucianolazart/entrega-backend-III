...
const getAdoption = async(req,res)=>{
    ...
    if(!adoption) return res.status(404).send({status:"error",error:"Adopción no encontrada"})
...
const createAdoption = async(req,res)=>{
    ...
    if(pet.adopted) return res.status(400).send({status:"error",error:"La mascota ya está adoptada"});
